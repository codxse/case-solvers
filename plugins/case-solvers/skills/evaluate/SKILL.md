...
- **Implementation needs work (most common)** → the contract is fine, the code isn't. Don't bounce the story back to `/solve`; fix it in place by delegating the review to a **frontier model**:
  1. **Spawn the review-and-apply as a subagent and pin its model to a frontier tier explicitly — never inherit the ambient model.** `/evaluate` carries no model gate, so if you don't set the model the subagent inherits whatever `/evaluate` is running on — often a budget model like `haiku` — which is exactly the failure this step exists to prevent. Pinning is **mandatory**, not best-effort: spawn via the subagent tool with its `model` parameter set, defaulting to **`opus`** (the preferred frontier reviewer). Concretely on Claude Code, the spawn must carry `model: "opus"` (e.g. `Agent(subagent_type: "general-purpose", model: "opus", prompt: …)`) — the `model` argument is required here, not optional. `sonnet` is an acceptable frontier fallback only when `opus` is unavailable; on Codex, pin its frontier GPT-5-class equivalent. **Never** pin or inherit a budget ID (`haiku`/`flash`/`mini`/`lite`/`nano`/…), and never run the review inline on `/evaluate`'s own model instead of spawning. If no frontier model can be pinned, **stop** and tell the user — do not fall back to a budget reviewer.
     **Under `--unattended`, the pin is tier-keyed instead of a flat default** — an orchestrated epic pays for many reviews, so each one costs what its own story's Complexity call says it warrants, expressed relative to the host's frontier roster, never as hardcoded model names (the roster changes; the rule shouldn't):
     - `solver-budget` or `solver-medium` (from the story's labels, already in step 1's `bd show`) → the **cheapest model on the host's frontier roster** (Claude: `sonnet`; Codex: its base GPT-5-class tier). One exception: if the recorded assignee (the claim in `bd show`) is **the same model class as this chosen reviewer** — the story was solved by `sonnet` and the reviewer would also be `sonnet` — step up to the strongest instead, so a model never reviews its own class's work. A budget-roster assignee (`haiku`, `flash`, `mini`, …) never triggers this step-up: the cheapest frontier reviewer checking a budget solver's work is exactly the intended cheap path, not a conflict.
     - `solver-frontier`, or no `solver-*` label → the **strongest frontier model the host offers** (Claude: `opus`; Codex: its strongest GPT-5-class).
     The floor never moves: every choice stays on the frontier roster, so the never-a-budget-ID rule above binds unattended runs identically. A host whose roster has only one frontier option → both rungs point at it; the rule degrades to today's behavior, it never errors.
...
| request impl change | spawn subagent **pinned to a frontier model (`model: "opus"`; under `--unattended`, tier-keyed per step 4b.1 — cheapest frontier for `solver-budget`/`-medium`, strongest for `solver-frontier`; never inherited)** → `/code-review <effort> --fix` in `.worktree/<id>` (effort from `--review`, default `high`) → show applied diff + **confirm before amend (skipped under `--unattended`)** → amend `bd/<id>` (keep branch + `needs-review`) |
